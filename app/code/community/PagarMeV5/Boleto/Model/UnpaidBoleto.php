...
class PagarMeV5_Boleto_Model_UnpaidBoleto {
	private $logger;
	...
	private function expiredBoletos() {
		...
		$boletosFilter = Mage::getModel('pagarmev5_core/transaction')->getCollection();
	...
	private function loadBoletoTransaction($orderId) {
		$sdk = Mage::getModel('pagarmev5_core/sdk_adapter')->getSdk();
	...
	private function cancelOrder(Mage_Sales_Model_Order $order, GetOrderResponse $orderResponse) {
		if ($order->getState() === Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
			$cancelHandler = new PagarMeV5_Core_Model_OrderStatusHandler_UnpaidBoleto(
				$order,
				$orderResponse
			);
