...
class PagarMeV5_Core_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	public function prepareAddressData($shippingAddress) {
		...
		$addressRequest->state = $shippingAddress->getRegionCode();
		$addressRequest->country = $shippingAddress->getCountry();

		//$addressRequest->line1 = $shippingAddress->getStreet(2) . ', ' . $shippingAddress->getStreet(1);
		if (!$helper->isEmpty($shippingAddress->getStreet(4))) {
			$addressRequest->neighborhood = $shippingAddress->getStreet(4);
			//$addressRequest->line1 .= ', ' . $shippingAddress->getStreet(4);
		}

		if (!$helper->isEmpty($shippingAddress->getStreet(3))) {
			$addressRequest->line2 = $shippingAddress->getStreet(3);
			$addressRequest->neighborhood = $shippingAddress->getStreet(3);
		}
	...
	public function prepareOrderItems($order) {
		...
		foreach ($order->getAllVisibleItems() as $item) {
			...
			if (!empty($qtd) && $basePrice > 0) {
				...
				$orderItem->code = $item->getProductId();
				$orderItem->amount = $this->parseAmountToCents($basePrice);
				$orderItem->description = substr($item->getName(), 0, 255);
